...
# -*- coding: utf-8 -*-
"""
Lightweight EN-only multi-task fine-tuning dataset builder (downsized).

Changes vs previous:
- MAX_CONTEXT_CHARS = 800
- MIN/MAX_TASKS_PER_RECORD = 2/3
- MAX_BULLETS = 4
- MAX_FAQ = 2
- New downscaling flags: --sample_frac, --max_total_samples
- Same outputs and schemas as before.
"""
...
SEED = 3407
VAL_RATIO = 0.02                   # ~2% validation
MAX_CONTEXT_CHARS = 800            # ↓ from 1800
MAX_TITLE_CHARS = 70
MAX_BULLETS = 4                    # ↓ from 6
MAX_FAQ = 2                        # new: limit FAQ pairs
MIN_TASKS_PER_RECORD = 2           # ↓ from 3
MAX_TASKS_PER_RECORD = 3           # ↓ from 5
...
def make_faq(meta: Dict[str, Any], max_pairs=MAX_FAQ) -> List[Tuple[str, str]]:
    QAs: List[Tuple[str,str]] = []
    if meta.get("price_display") or meta.get("original_price_display"):
        QAs.append(("What is the price?",
                    _strip(meta.get("price_display") or meta.get("original_price_display")) or "Not available"))
    ...
    if not QAs:
        QAs.append(("What is this product?", _strip(meta.get("title")) or "Not available"))
    return QAs[:max_pairs]  # limit here
...
def build_samples_for_record(rec: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 2) Strict JSON extraction with normalization
    gold_json = make_json_norm(meta)
    samples.append({
        "messages":[
            {"role":"system","content":"You convert product context into strict JSON following the schema and do light normalization."},
    ...
    # 4) Feature bullets (Markdown)
    bullets_en = make_bullets(meta, MAX_BULLETS)
    samples.append({
        "messages":[
            {"role":"system","content":"You write crisp product bullets in Markdown."},
            {"role":"user","content":"List concise product bullets in Markdown based on the context (max 4).\n\n=== PRODUCT CONTEXT ===\n"+text},
            {"role":"assistant","content":"\n".join(f"- {b}" for b in bullets_en)}
        ]
    })

    # 5) FAQ (limited)
    for q, a in make_faq(meta, MAX_FAQ):
        samples.append({
            "messages":[
                {"role":"system","content":"You answer clearly and concisely."},
                {"role":"user","content":"Q: "+q+"\n\nUse the product context only.\n\n=== PRODUCT CONTEXT ===\n"+text},
    ...
    # 6) Compact Markdown table (kept as an option in the pool)
    table_md = make_markdown_table(meta)
    samples.append({
        "messages":[
            {"role":"system","content":"You produce compact Markdown tables without extra text."},
    ...
    # Random cap for size control (2–3 tasks)
    random.shuffle(samples)
    n = random.randint(MIN_TASKS_PER_RECORD, min(MAX_TASKS_PER_RECORD, len(samples)))
    return samples[:n]
...
def make_dpo_pair(meta: Dict[str, Any]) -> Optional[Dict[str, str]]:
    title_good = make_seo_title(meta, MAX_TITLE_CHARS)
    title_bad  = (make_seo_title(meta, 120) + " ⭐🔥 SUPER SALE!!!!")
    prompt = (
        "Create an SEO-friendly title under 70 characters for this product. "
        "No emojis, no clickbait, do not exceed 70 characters."
    )
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default=str(OUT_DIR))
    parser.add_argument("--make_dpo", action="store_true", help="Also emit ecom_dpo_pairs.jsonl")
    parser.add_argument("--max_products", type=int, default=None, help="Cap products processed (pre-build)")
    parser.add_argument("--sample_frac", type=float, default=None, help="Randomly sample this fraction of products (0–1)")
    parser.add_argument("--max_total_samples", type=int, default=None, help="Hard cap on total chat samples (post-build)")
    args = parser.parse_args()
    ...
    # Pre-build downscaling
    if args.sample_frac is not None:
        if not (0 < args.sample_frac <= 1):
            raise SystemExit("--sample_frac must be in (0, 1]")
        k = max(1, int(len(records) * args.sample_frac))
    ...
    # Post-build cap (if requested)
    if args.max_total_samples is not None and len(sft_rows) > args.max_total_samples:
        sft_rows = random.sample(sft_rows, args.max_total_samples)
    ...
    stats = {
        ...
        "max_context_chars": MAX_CONTEXT_CHARS,
        "caps": {
            "min_tasks_per_record": MIN_TASKS_PER_RECORD,
            "max_tasks_per_record": MAX_TASKS_PER_RECORD,
            "max_bullets": MAX_BULLETS,
            "max_faq": MAX_FAQ,
        },
        "downscale_flags_used": {
        ...
        "tasks_pool": [
            ...
            "Strict JSON extraction with normalization (EN)",
            f"SEO title (<= {MAX_TITLE_CHARS} chars) (EN)",
            f"Feature bullets (Markdown, EN; ≤{MAX_BULLETS})",
            f"FAQ Q&A (EN; ≤{MAX_FAQ})",
            "Compact Markdown table (EN)",
        ],
    }
    with STATS_F.open("w", encoding="utf-8") as w:
